...
class NrwServicesPageMeta {
	...
	public function __construct() {
		...
		for($i = 0; $i < $services; $i++) {
			$j = $i + 1;
			$this->meta_groups[] = array(
				'name'         => __('Service ' . $j, NRW_TXT_DOMAIN),
				'fields'       => array(
					array(
						'type'           => 'text',
						'name'           => 'service_' . $j . '_title',
						'id'             => 'service_' . $j . '_title',
						'label'          => __('Service Title ' . $j, NRW_TXT_DOMAIN),
						'description'    => ''
					),
					array(
						'type'         => 'image',
						'name'         => 'service_' . $j . '_icon',
						'id'           => 'service_' . $j . '_icon',
						'btn_id'       => 'btn_service_' . $j . '_icon',
						'label'        => __('Service Image ' . $j, NRW_TXT_DOMAIN),
						'description'  => ''
					),
					array(
						'type'         => 'textarea',
						'name'         => 'service_' . $j . '_content',
						'id'           => 'service_' . $j . '_content',
						'label'        => __('Service Content ' . $j, NRW_TXT_DOMAIN),
						'description'  => ''
					),
					array(
						'type'         => 'select',
						'name'         => 'service_' . $j . '_link_id',
						'id'           => 'service_' . $j . '_link_id',
						'choices'      => NrwOptions::get_all_pages(),
						'label'        => __( 'Service Link ' . $j, NRW_TXT_DOMAIN),
						'description'  => ''
					)
				)
			);
	...
	public function add_meta_boxes() {
		add_meta_box(
			'nrw_services_template',
			__('Services', NRW_TXT_DOMAIN),
	...
	public function render_meta_boxes( $post ) {
		wp_nonce_field('hp_services_nonce_action', 'hp_services_nonce');
		echo NrwCore::render_meta_boxes($this->meta_groups, get_post_meta($post->ID));
	}

	public function save_meta_data($post_id) {
		$nonce_name   = isset( $_POST['hp_services_nonce'] ) ? $_POST['hp_services_nonce'] : '';
		$nonce_action = 'hp_services_nonce_action';
...
$nrwservicespagemeta = new NrwServicesPageMeta();
echo get_page_template_slug();
if ( isset( $_GET['post'] ) ) {
	$slug = get_post_meta($_GET['post'], '_wp_page_template', true);
	if('page-services.php' == $slug ) {
		$nrwservicespagemeta->init();
	}
}
$nrwservicespagemeta->init_save();